...
class SwinTransformerSys(nn.Module):
    """
    3-branch encoder + bottleneck fusion + original decoder (skip from base only)

    Base: patch_size=4,  C=48  -> bottleneck dim = 8C  = 384 (W/32,H/32)
    cp1 : patch_size=2,  C1=12 -> bottleneck dim = 16C1= 192 (W/32,H/32)
    cp2 : patch_size=8,  C2=192-> bottleneck dim = 4C2 = 768 (W/32,H/32)

    Fusion at bottleneck:
        cat([base, cp1, cp2]) -> Linear -> back to base bottleneck dim (384)

    Decoder:
        giữ nguyên logic decoder gốc, dùng skip connections của base branch.
    """
    ...
    def __init__(
        ...
        # base config (config yaml có thể truyền vào, nhưng base dim sẽ bị override về 48)
        embed_dim=96,
        depths=(2, 2, 2, 2),
        num_heads=(3, 6, 12, 24),
        ...
        **kwargs
        ...
        # ===== fixed branch dims as you requested =====
        self.C_base = 48
        self.C1 = 12
        self.C2 = 192

        # override base embed_dim
        embed_dim = self.C_base

        self.num_classes = num_classes
        self.num_layers = len(depths)  # base stages count (typically 4)
        self.embed_dim = embed_dim
        self.ape = ape
        ...
        self.num_features = int(embed_dim * 2 ** (self.num_layers - 1))  # 48*8 = 384 when num_layers=4
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size,
            patch_size=patch_size,  # should be 4 for base
            in_chans=in_chans,
            embed_dim=embed_dim,
        ...
        # =========================
        # 2) BASE ENCODER (same style as original)
        # =========================
        ...
        num_heads_cp1 = (3, 3, 6, 12, 24)       # choose something valid
        ...
        num_heads_cp2 = (6, 12, 24)    # choose something valid
        ...
        # =========================
        # 6) DECODER (keep original style; skips from base)
        # =========================
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x_downsample.append(x)  # base skips only
            x = layer(x)

        x = self.norm(x)  # (B, L32, 384)
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x = torch.cat([x, x_downsample[skip_idx]], dim=-1)
    ...
    def forward(self, x):
        ...
        # cp branches (bottleneck only)
        x_cp1 = self.forward_features_cp1(x)
        x_cp2 = self.forward_features_cp2(x)

        # bottleneck fusion -> back to base dim (384)
        x_fused = torch.cat([x_base, x_cp1, x_cp2], dim=-1)  # (B, L32, 1344)
        x = self.fuse_to_base(x_fused)                      # (B, L32, 384)

        # decoder uses base skips only
